      ...
      <div class="cart-foot-wrap">
        ...
        <div class="cart-foot-r">
          ...
          <div class="next-btn-wrap">
            <a href="javascript:;" @click="checkNum()" class="btn btn--red" style="width: 200px">结账</a>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  },//实例化创建完成以后，编译完成后，默认查询某一个方法，需要定义一个mounted
  methods:{
    cartView: function(){
      var _this = this;//在所有的vue实例方法里面，所有的this都指向vm实例，函数内部会发生变化
      var params ={
        api:'static/data/cartData.json',
        param:{
          id:123
        }
      };
...
//      this.$http.get("n",{"id":123}).then(res=>{
//        this.productList = res.data.result.list;
//      this.totalMoney = res.data.result.totalMoney;
//    })//resource插件基于vue，在vm实例里调出http方法
    },
    ...
    selectedProduct: function(item){
      if(typeof item.checked == 'undefined'){
        // Vue.set(item,"checked",true)//往item里注册checked属性，其值为true
    ...
    delProduct: function(){
      ...
      this.productList.splice(index,1);//从index开始，删除一个元素
      this.delFlag = false;
    },
    checkNum: function(){
      console.log(this.totalMoney==0);
